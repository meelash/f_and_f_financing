  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
      <section className="card p-6">
        <h1 className="text-2xl font-semibold">First-Time Setup Wizard</h1>
        <p className="mt-2 text-sm text-black/70">
          Create your first partnership and login accounts. Setup passwords are temporary and must be changed on first sign in.
        </p>
      </section>
      ...
      <form className="card grid gap-4 p-6" onSubmit={onSubmit}>
        ...
        <h2 className="mt-2 text-lg font-medium">Occupant (Primary Admin)</h2>
        <div className="grid gap-3 sm:grid-cols-2">
        ...
        <button disabled={busy} className="mt-2 rounded bg-[var(--surface-strong)] px-4 py-2 font-medium text-white disabled:opacity-60">
          {busy ? "Initializing..." : "Create Partnership"}
        </button>
      ...
      {result ? (
        <section className="card p-6">
          ...
          <p className="mt-2 text-sm text-black/70">Setup saved. Redirecting to login...</p>
        </section>
      ) : null}
    </main>
  );
